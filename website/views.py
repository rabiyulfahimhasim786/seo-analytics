...
from .serializers import PageperformanceSerializer, MobileSerializer, OverallSerializer
...
class MobileView(ListCreateAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Return an error response with the URL and error message
            error_msg = f"HTTP error occurred: {http_err}"
            error_data = {'url': mobile_url, 'error': error_msg}
            return Response(error_data, status=status.HTTP_400_BAD_REQUEST)
...
class OverallView(ListCreateAPIView):
    queryset = Overallperformance.objects.all()
    serializer_class = OverallSerializer

    def post(self, request):
        input_url = request.data.get('urls')
        if not input_url:
            input_url = request.query_params.get('urls')
        # api_key = "your-api-key"
        strategydesktop = "desktop"  # or "desktop" OR "mobile" 
        strategymobile ="mobile"
        try:
            # Use the PageSpeed Insights API to fetch a report on website performance, best practices, accessibility, and PWA
            psi_url = f"https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={input_url}&key={pageinsights_api_key}&strategy={strategymobile}&category=performance&category=accessibility&category=seo&category=best-practices"  
            #psi_url = f"https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={url}&key={api_key}&strategy={strategy}&category=performance,accessibility,best-practices,seo,pwa"
            response = requests.get(psi_url)
            report = response.json()
            ...
            print(output_data)
            # print(x)
            #page perfromace desktop
            desktops_url = f"https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={input_url}&key={pageinsights_api_key}&strategy={strategydesktop}&category=performance&category=accessibility&category=seo&category=best-practices"  
            #psi_url = f"https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url={url}&key={api_key}&strategy={strategy}&category=performance,accessibility,best-practices,seo,pwa"
            desktopresponse = requests.get(desktops_url)
            desktopreport = desktopresponse.json()

            # Print the report
            # print(report)

            # Serializing json
            desktopjson_object = json.dumps(desktopreport, indent=4)
            
            # Writing to sample.json
            # print(dot)
            desktopfilepath = dot+"desktop.json"
            with open(desktopfilepath, "w") as desktopoutfile:
                desktopoutfile.write(desktopjson_object)
            with open(desktopfilepath) as desktopf:
                desktopdata = json.load(desktopf)
                #print(data)
                dperformance_score =  desktopdata["lighthouseResult"]["categories"]["performance"]["score"]* 100
                daccessibility_score = desktopdata["lighthouseResult"]["categories"]["accessibility"]["score"]* 100
                dbest_practices_score =  desktopdata["lighthouseResult"]["categories"]["best-practices"]["score"]* 100
                dseo_score = desktopdata["lighthouseResult"]["categories"]["seo"]["score"]* 100
                # print(performance_score)
                # print(accessibility_score)
                # print(best_practices_score)
                # print(seo_score)
            y = {"performance": round(dperformance_score), "accessibility": round(daccessibility_score), "best-practices":round(dbest_practices_score), "seo": round(dseo_score)}
            # output_data = json.dumps(x, indent = 5) 
            desktop_data = json.dumps(y, indent = 4) 
            print(desktop_data)

            overallseo = Overallperformance.objects.create(urls=input_url, mobile=output_data, desktop=desktop_data)
            # print(output_data)
            # serializer = PageperformanceSerializer(seo)
            serializer = OverallSerializer(overallseo)
            return Response(serializer.data)
...
class OverallUpdateDeleteApiView(RetrieveUpdateDestroyAPIView):
    queryset = Overallperformance.objects.all()
    serializer_class = OverallSerializer
    #permission_classes = [IsAuthenticated]
    permission_classes = [AllowAny]